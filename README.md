...
|-----------|-------------|
| model | Model architecture name. Options include:<br>• seq2seq_lstm – (deterministic) sequence-to-sequence LSTM baseline<br>• encdec_lstm – (deterministic) encoder–decoder LSTM baseline<br>• seq2seq_ssm – (deterministic) state-space model (S4D-FT)<br>• decoder_only_ssm – HydroDiffusion model<br>• decoder_only_ssm_hybrid – faster inference for HydroDiffusion using hybrid FFT+recurrent decoding (~17x faster, numerically equivalent to decoder_only_ssm)<br>• diffusion_lstm – DiffusionLSTM_encdec (encoder-decoder LSTM diffusion model)<br>• diffusion_lstm_fast – faster inference for DiffusionLSTM_encdec; encoder runs once and is cached across all ensemble members<br>• decoder_only_lstm – DiffusionLSTM_dec (decoder-only LSTM diffusion model)<br>• decoder_only_lstm_fast – faster inference for DiffusionLSTM_dec; past LSTM encoding is cached once and reused across all ensemble members |
...
2. Launch training using 5 dynamic meteorological forcings from Daymet and 27 static catchment attributes.  
...
### Fast Inference Variants

The `_fast` and `_hybrid` model variants share the same weights as their original counterparts and produce numerically equivalent outputs. They can be used directly with a checkpoint trained under the original model name:

```
./test.sh decoder_only_ssm_hybrid static 0 runs/run_2507_2120_seed3407 myExperiment
./test.sh decoder_only_lstm_fast   static 0 runs/run_<MMDD>_<HHMM>_seed3407 myExperiment
./test.sh diffusion_lstm_fast      static 0 runs/run_<MMDD>_<HHMM>_seed3407 myExperiment
```
...
|-------------|--------------|--------------|
| DiffusionLSTM<sub>EncDec</sub> (`diffusion_lstm`, `diffusion_lstm_fast`) | `ensembles_epochbest.npz` | Full ensemble forecasts `(N, S, H)` generated from the best-performing epoch. |
| DiffusionLSTM<sub>Dec</sub> (`decoder_only_lstm`, `decoder_only_lstm_fast`) | `ensembles_epochbest.npz` | Full ensemble forecasts `(N, S, H)` generated from the best-performing epoch. |
| HydroDiffusion (`decoder_only_ssm`, `decoder_only_ssm_hybrid`) | `ensembles_epoch60.npz` | Full ensemble forecasts `(N, S, H)` generated from epoch 60. |
